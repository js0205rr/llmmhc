...
class MDataset(Dataset):
    def __init__(self, df, mode, tokenizer, label_map, max_length,
        ...
        if group_y is not None:
            ...
            self.map_group_y = np.empty(len(label_map), dtype=np.int_)
    ...
    def __getitem__(self, idx):
        ...
        if self.token_type_ids is not None:
            ...
            # 统一使用 tokenizer 的 __call__ 方法，兼容所有模型
            encoding = self.tokenizer(
                'filling empty' if len(text) == 0 else text,
                add_special_tokens=True,
                max_length=max_len,
                truncation=True,
                padding=False,          # 不在此处 padding，后面统一处理
                return_tensors=None     # 返回 Python 列表
            )
            input_ids = encoding['input_ids']
            # 如果返回的是嵌套列表（batch=1），取第一个元素
