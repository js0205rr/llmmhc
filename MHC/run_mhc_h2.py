...
from models.resnet_v1 import ResNet18
...
def test(epoch):
    ...
    for batch_idx, (inputs, targets) in tqdm(enumerate(testloader), total=len(testloader)):
        ...
        with torch.no_grad():
            inputs, targets = Variable(inputs), Variable(targets)
        local_logit, g_logits = net(inputs)

        g_label = targets // args.num_classes[1]
        local_label = targets % args.num_classes[1]
        pred = torch.argmax(local_logit.data, -1).eq(local_label.data)
        g_pred = torch.argmax(g_logits.data, -1).eq(g_label.data)
        pred = pred & g_pred
