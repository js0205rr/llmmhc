"""
Tree-based label organization for extreme classification.

This module implements the ECLARE tree structure for extreme classification,
which provides efficient label organization with parallel processing capabilities.
"""

import numpy as np
import torch
import copy
import time
import scipy.sparse as sp
from sklearn.preprocessing import normalize as scale
from functools import partial, reduce
import operator
import _pickle as pik
import tqdm
from multiprocessing import Pool, cpu_count
from typing import List, Tuple, Union, Optional, Callable, Dict


def _normalize(X: np.ndarray, norm: str = 'l2') -> np.ndarray:
    """
    Normalize input array using specified norm.
    
    Args:
        X: Input array to normalize
        norm: Normalization method ('l2' by default)
        
    Returns:
        Normalized array
    """
    return scale(X, norm=norm)


def b_kmeans_dense_multi(fts_lbl: np.ndarray, 
                        index: np.ndarray, 
                        metric: str = 'cosine', 
                        tol: float = 1e-4, 
                        leakage: Optional[float] = None) -> List[np.ndarray]:
    """
    Binary k-means clustering for multi-objective dense features.
    
    Args:
        fts_lbl: Feature matrix with shape (n_samples, 2, n_features)
        index: Indices of labels
        metric: Distance metric to use
        tol: Convergence tolerance
        leakage: Optional leakage parameter
        
    Returns:
        List of two clusters containing label indices
    """
    lbl_cent = _normalize(np.squeeze(fts_lbl[:, 0, :]))
    lbl_fts = _normalize(np.squeeze(fts_lbl[:, 1, :]))
    
    if lbl_cent.shape[0] == 1:
        return [index]
        
    # Initialize clusters
    cluster = np.random.randint(low=0, high=lbl_cent.shape[0], size=(2))
    while cluster[0] == cluster[1]:
        cluster = np.random.randint(low=0, high=lbl_cent.shape[0], size=(2))
    
    _centeroids = lbl_cent[cluster]
    _sim = np.dot(lbl_cent, _centeroids.T)
    old_sim, new_sim = -1000000, -2
    
    # Iterative clustering
    while new_sim - old_sim >= tol:
        c_lbs = np.array_split(np.argsort(_sim[:, 1]-_sim[:, 0]), 2)
        
        # Update centroids for both objectives
        _centeroids = _normalize(np.vstack([
            np.mean(lbl_cent[x, :], axis=0) for x in c_lbs
        ]))
        _sim_1 = np.dot(lbl_cent, _centeroids.T)
        
        _centeroids = _normalize(np.vstack([
            np.mean(lbl_fts[x, :], axis=0) for x in c_lbs
        ]))
        _sim_2 = np.dot(lbl_fts, _centeroids.T)
        
        _sim = _sim_1 + _sim_2
        old_sim, new_sim = new_sim, np.sum([
            np.sum(_sim[c_lbs[0], 0]),
            np.sum(_sim[c_lbs[1], 1])
        ])
    
    return list(map(lambda x: index[x], c_lbs))


def b_kmeans_dense(labels_features: np.ndarray, 
                  index: np.ndarray, 
                  metric: str = 'cosine', 
                  tol: float = 1e-4, 
                  leakage: Optional[float] = None) -> List[np.ndarray]:
    """
    Binary k-means clustering for dense features.
    
    Args:
        labels_features: Feature matrix
        index: Indices of labels
        metric: Distance metric to use
        tol: Convergence tolerance
        leakage: Optional leakage parameter
        
    Returns:
        List of two clusters containing label indices
    """
    labels_features = _normalize(labels_features)
    if labels_features.shape[0] == 1:
        return [index]
        
    # Initialize clusters
    cluster = np.random.randint(low=0, high=labels_features.shape[0], size=(2))
    while cluster[0] == cluster[1]:
        cluster = np.random.randint(low=0, high=labels_features.shape[0], size=(2))
    
    _centeroids = labels_features[cluster]
    _similarity = np.dot(labels_features, _centeroids.T)
    old_sim, new_sim = -1000000, -2
    
    # Iterative clustering
    while new_sim - old_sim >= tol:
        clustered_lbs = np.array_split(
            np.argsort(_similarity[:, 1]-_similarity[:, 0]), 2)
        
        _centeroids = _normalize(np.vstack([
            np.mean(labels_features[x, :], axis=0) for x in clustered_lbs
        ]))
        _similarity = np.dot(labels_features, _centeroids.T)
        
        old_sim, new_sim = new_sim, np.sum([
            np.sum(_similarity[indx, i]) 
            for i, indx in enumerate(clustered_lbs)
        ])
    
    return list(map(lambda x: index[x], clustered_lbs))


def b_kmeans_sparse(labels_features: sp.spmatrix, 
                   index: np.ndarray, 
                   metric: str = 'cosine', 
                   tol: float = 1e-4, 
                   leakage: Optional[float] = None) -> List[np.ndarray]:
    """
    Binary k-means clustering for sparse features.
    
    Args:
        labels_features: Sparse feature matrix
        index: Indices of labels
        metric: Distance metric to use
        tol: Convergence tolerance
        leakage: Optional leakage parameter
        
    Returns:
        List of two clusters containing label indices
    """
    labels_features = _normalize(labels_features)
    if labels_features.shape[0] == 1:
        return [index]
        
    # Initialize clusters
    cluster = np.random.randint(low=0, high=labels_features.shape[0], size=(2))
    while cluster[0] == cluster[1]:
        cluster = np.random.randint(low=0, high=labels_features.shape[0], size=(2))
    
    _centeroids = _normalize(labels_features[cluster].todense())
    _sim = _sdist(labels_features, _centeroids)
    old_sim, new_sim = -1000000, -2
    
    # Iterative clustering
    while new_sim - old_sim >= tol:
        c_lbs = np.array_split(np.argsort(_sim[:, 1]-_sim[:, 0]), 2)
        _centeroids = _normalize(np.vstack([
            labels_features[x, :].mean(axis=0) for x in c_lbs
        ]))
        _sim = _sdist(labels_features, _centeroids)
        old_sim, new_sim = new_sim, np.sum([
            np.sum(_sim[c_lbs[0], 0]), 
            np.sum(_sim[c_lbs[1], 1])
        ])
    
    return list(map(lambda x: index[x], c_lbs))


def _sdist(XA: sp.spmatrix, XB: np.ndarray, norm: Optional[str] = None) -> np.ndarray:
    """
    Compute sparse distance between matrices.
    
    Args:
        XA: Sparse matrix
        XB: Dense matrix
        norm: Optional normalization method
        
    Returns:
        Distance matrix
    """
    return XA.dot(XB.transpose())


def _merge_tree(cluster: List[np.ndarray], 
                verbose_label_index: List[np.ndarray], 
                avg_size: int = 0, 
                force: bool = False) -> Tuple[List[np.ndarray], List[np.ndarray]]:
    """
    Merge tree clusters with verbose labels.
    
    Args:
        cluster: List of clusters
        verbose_label_index: List of verbose label indices
        avg_size: Average size parameter
        force: Whether to force merge
        
    Returns:
        Tuple of merged clusters and remaining verbose labels
    """
    if cluster[0].size < verbose_label_index[0].size:
        print("Merging trees", np.log2(len(cluster)))
        return cluster + verbose_label_index, [np.asarray([])]
    elif verbose_label_index[0].size > 0 and force:
        if verbose_label_index.shape[0] > 0:
            print("Force Merging trees")
            return cluster + verbose_label_index, [np.asarray([])]
        else:
            print("Nothing else to do")
            return cluster, [np.asarray([])]
    else:
        return cluster, verbose_label_index


def cluster_labels_parallel(labels: Union[np.ndarray, List[np.ndarray]], 
                          clusters: List[np.ndarray], 
                          verbose_label_index: List[np.ndarray], 
                          num_nodes: int, 
                          splitter: Callable) -> Tuple[List[np.ndarray], List[np.ndarray]]:
    """
    Parallel implementation of label clustering.
    
    Args:
        labels: Label features or list of label features
        clusters: Initial clusters
        verbose_label_index: Verbose label indices
        num_nodes: Target number of nodes
        splitter: Function to split clusters
        
    Returns:
        Tuple of final clusters and verbose labels
    """
    start = time.time()
    least = min(16, num_nodes)
    clusters, verbose_label_index = _merge_tree(clusters, verbose_label_index)
    
    with Pool(cpu_count()-1) as p:
        while len(clusters) < num_nodes:
            if isinstance(labels, list):
                temp_cluster_list = reduce(
                    operator.iconcat,
                    p.starmap(splitter, 
                             map(lambda x: (labels[0][x], labels[1][x], x), 
                                 clusters)), [])
            else:    
                temp_cluster_list = reduce(
                    operator.iconcat,
                    p.starmap(splitter, 
                             map(lambda x: (labels[x], x), 
                                 clusters)), [])

            end = time.time()
            print("Total clusters {}".format(len(temp_cluster_list)),
                  "Avg. Cluster size {}".format(
                      np.mean(list(map(len, temp_cluster_list+verbose_label_index)))),
                  "Total time {} sec".format(end-start))
            
            clusters = temp_cluster_list
            clusters, verbose_label_index = _merge_tree(clusters, verbose_label_index)
            del temp_cluster_list
            
    return clusters, verbose_label_index


def representative(lbl_fts: np.ndarray) -> np.ndarray:
    """
    Find representative feature vector.
    
    Args:
        lbl_fts: Label features
        
    Returns:
        Representative feature vector
    """
    scores = np.ravel(np.sum(np.dot(lbl_fts, lbl_fts.T), axis=1))
    return lbl_fts[np.argmax(scores)]


class hash_map_index:
    """Hash map for label indices with optional padding."""
    
    def __init__(self, 
                 clusters: Optional[List[np.ndarray]], 
                 label_to_idx: np.ndarray, 
                 total_elements: int, 
                 total_valid_nodes: int, 
                 padding_idx: Optional[int] = None):
        """
        Initialize hash map.
        
        Args:
            clusters: List of clusters
            label_to_idx: Mapping from labels to indices
            total_elements: Total number of elements
            total_valid_nodes: Number of valid nodes
            padding_idx: Optional padding index
        """
        self.clusters = clusters
        self.padding_idx = padding_idx
        self.total_elements = total_elements
        self.size = total_valid_nodes
        self.weights = None
        
        if padding_idx is not None:
            self.weights = np.zeros((self.total_elements), np.float)
            self.weights[label_to_idx == padding_idx] = -np.inf
        
        self.hash_map = label_to_idx
    
    def _get_hash(self) -> np.ndarray:
        """Get hash map."""
        return self.hash_map
    
    def _get_weights(self) -> Optional[np.ndarray]:
        """Get weights."""
        return self.weights


class EclareTree:
    """
    ECLARE tree implementation for extreme classification with parallel processing.
    
    This class provides efficient label organization with parallel processing capabilities
    and support for multi-objective label features.
    """
    
    def __init__(self, 
                 b_factors: List[int] = [2], 
                 M: int = 1, 
                 method: str = 'random',
                 leaf_size: int = 0, 
                 force_shallow: bool = True):
        """
        Initialize ECLARE tree.
        
        Args:
            b_factors: Branching factors
            M: Number of trees
            method: Clustering method
            leaf_size: Maximum leaf size
            force_shallow: Whether to force shallow trees
        """
        self.b_factors = b_factors
        self.C = []
        self.method = method
        self.leaf_size = leaf_size
        self.force_shallow = force_shallow
        self.height = 2

    def fit(self, 
            label_index: List[int] = [], 
            verbose_label_index: List[int] = [], 
            lbl_repr: Optional[Union[np.ndarray, sp.spmatrix]] = None):
        """
        Fit ECLARE tree to data.
        
        Args:
            label_index: Label indices
            verbose_label_index: Verbose label indices
            lbl_repr: Label representations
        """
        self.num_labels = lbl_repr.shape[0]
        clusters = [label_index]
        self.hash_map_array = []
        print("Total verbose labels", verbose_label_index.size)

        # Select appropriate clustering method
        if len(lbl_repr.shape) > 2:
            print("Using multi objective kmeans++")
            b_kmeans = b_kmeans_dense_multi
        elif isinstance(lbl_repr, np.ndarray):
            print("Using dense kmeans++")
            b_kmeans = b_kmeans_dense
        else:
            lbl_repr = lbl_repr.tocsr()
            b_kmeans = b_kmeans_sparse

        if self.method == "NoCluster":
            self.height = 1
            print("No need to create splits")
            n_lb = self.num_labels
            self.hash_map_array.append(hash_map_index(
                None, np.concatenate(clusters), n_lb, n_lb, n_lb))
            return

        self._build_tree(lbl_repr, clusters, [verbose_label_index],
                        b_kmeans, self.force_shallow)

    def _build_tree(self, 
                    labels: Union[np.ndarray, sp.spmatrix], 
                    clusters: List[np.ndarray], 
                    verbose_label_index: List[np.ndarray],
                    splitter: Callable, 
                    force_shallow: bool = True):
        """
        Build ECLARE tree structure with parallel processing.
        
        Args:
            labels: Label features
            clusters: Initial clusters
            verbose_label_index: Verbose label indices
            splitter: Function to split clusters
            force_shallow: Whether to force shallow trees
        """
        depth = 0
        T_verb_lbl = verbose_label_index[0].size
        
        while True:
            orignal_num_nodes = 2**self.b_factors[depth]
            n_child_nodes = orignal_num_nodes
            
            if self.num_labels/n_child_nodes < T_verb_lbl or \
                    len(self.b_factors) == 1:
                if T_verb_lbl > 0:
                    add_at = np.floor(np.log2(self.num_labels/T_verb_lbl))+1
                    addition = 2**(self.b_factors[depth]-add_at)
                    n_child_nodes += addition
                    
            depth += 1
            print("Building tree at height %d with nodes: %d" %
                  (depth, n_child_nodes))
            
            if n_child_nodes >= self.num_labels:
                print("No need to do clustering")
                clusters = list(np.arange(self.num_labels).reshape(-1, 1))
            else:
                clusters, verbose_label_index = cluster_labels_parallel(
                    labels, clusters, verbose_label_index,
                    orignal_num_nodes, splitter)
                if depth == len(self.b_factors):
                    clusters, verbose_label_index = _merge_tree(
                        clusters, verbose_label_index, True)
                        
            self.hash_map_array.append(
                hash_map_index(
                    clusters,
                    np.arange(n_child_nodes),
                    n_child_nodes,
                    n_child_nodes
                )
            )
    
    def _get_cluster_depth(self, depth: int) -> hash_map_index:
        """Get clusters at specified depth."""
        return self.hash_map_array[depth]
    
    def load(self, fname: str):
        """Load tree from file."""
        with open(fname, 'rb') as f:
            self.__dict__.update(pik.load(f))
    
    def save(self, fname: str):
        """Save tree to file."""
        with open(fname, 'wb') as f:
            pik.dump(self.__dict__, f)
