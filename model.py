...
def get_bert(bert_name, path, use_lora=False, lora_r=16, lora_alpha=32, lora_dropout=0.1):
    if 'qwen' in bert_name.lower():
        print(f'load Qwen model: {bert_name}')
        ...
        bert = RobertaModel.from_pretrained(path+'roberta-base', config=model_config)
    elif 'xlnet' in bert_name:
        print('load xlnet-base-cased')
        model_config = XLNetConfig.from_pretrained(path+'xlnet-base-cased')
        model_config.output_hidden_states = True
        bert = XLNetModel.from_pretrained(path+'xlnet-base-cased', config=model_config)
    else:
        print('load bert-base-uncased')
        model_config = BertConfig.from_pretrained(path+'bert-base-uncased')
        model_config.output_hidden_states = True
...
class XMLMHE(nn.Module):
    def __init__(self, n_labels, group_y=None, bert='bert-base', bert_path='../NLP-Model/', feature_layers=5, dropout=0.5, update_count=1,
        ...
        self.bert_name, self.bert = bert, get_bert(bert, bert_path, use_lora, lora_r, lora_alpha, lora_dropout)
    ...
    def forward(self, input_ids, attention_mask, token_type_ids,
        ...
        if is_training:
            target_candidates = labels[labels>-1].detach().cpu()
            target_candidates_num  = (labels>-1).sum(-1).detach().cpu()
    ...
    def get_fast_tokenizer(self):
        if 'roberta' in self.bert_name:
            ...
            tokenizer = AutoTokenizer.from_pretrained(self.bert_name, trust_remote_code=True)
        else:
    ...
    def get_tokenizer(self):
        if 'roberta' in self.bert_name:
            ...
            print(f'load Qwen tokenizer: {self.bert_name}')
            tokenizer = AutoTokenizer.from_pretrained(self.bert_name,trust_remote_code=True)
        else:
    ...
    def train_epoch(self, epoch, dataloader, optimizer,
        ...
        with torch.set_grad_enabled(mode == 'train'):
            for step, data in enumerate(dataloader):
                ...
                if step % eval_step == 0 and eval_loader is not None and step != 0:
                    results = self.eval_epoch(epoch, eval_loader,log)
